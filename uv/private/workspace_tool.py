...
def _generate_pyproject(
    ...
    if requires_python:
        lines.append(f'project.requires-python = "{requires_python}"')
...
def _link_bundled_python(
    ...
    assert (bundle_dir / "python").is_dir()
...
def _install_host_python_shim(bundle_dir: pathlib.Path, minor: int) -> None:
    ...
    # python3 at runtime, so the bundle artifact has no dangling symlinks.
    bin_dir = bundle_dir / "bin"
    _clear_python_links(bin_dir)
    shim_path = bin_dir / "python3"
    shim_path.write_text(
        rf"""#!/bin/bash
# Auto-generated by rules_uv_bare for bundle_interpreter=False.
...
# points into the bundle bin/, which is how Python locates pyvenv.cfg and
...
def _prime_uv_interpreter_cache(
    uv_path: str, bundle_dir: pathlib.Path, platform_cache: str
) -> None:
    ...
    When the target is `linux-*` and the host is macOS, the probe enters its glibc/musl detection branch and fails with:
        ...
        error: Can't use Python at `<bundle_dir/bin/python3>`
...
def cmd_bundle(manifest: Manifest, config: BundleConfig) -> None:
    uv_path = str(pathlib.Path(config["uv_path"]).resolve())
    # bundle_dir must be absolute since uv resolves UV_PROJECT_ENVIRONMENT
    # relative to --project.
    bundle_dir = pathlib.Path(config["bundle_dir_path"]).resolve()
    ...
    bundle_target_platform = config["bundle_target_platform"]
    if bundle_target_platform:
        target_key = f"{host.implementation}-{host.version}-{bundle_target_platform}"
        target = _resolve_uv_python(uv_path, target_key)
        is_cross = (host.os, host.arch) != (target.os, target.arch)

    bundle_interpreter = config["bundle_interpreter"]
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if bundle_interpreter:
            target_python_bin = _install_python(
                uv_path, bundle_dir / "python", target.key
            )
        else:
            # Install the target interpreter into the temp dir.
            # Even if `bundle_interpreter` is False, the target python is needed to read sysconfig data.
            target_python_bin = _install_python(
                uv_path, wdir / ".target_python", target.key
            )

        # WARNING: host_python_bin must be a NON-managed interpreter. Creating the
        # venv with a uv-managed interpreter would make `uv sync` remove bundle_dir.
        ...
        # Create venv into bundle_dir with **host_python_bin**.
        _create_relocatable_venv(uv_path, bundle_dir, str(host_python_bin))

        build_env["UV_PROJECT_ENVIRONMENT"] = str(bundle_dir)
        sync_cmd = [
            str(uv_path),
            "sync",
            "--frozen",
        ...
        if is_cross:
            ...
            _prime_uv_interpreter_cache(uv_path, bundle_dir, platform_cache)

        subprocess.check_call(sync_cmd, env=build_env)

        if bundle_interpreter:
            python_dir_name = target_python_bin.parents[1].name
            _link_bundled_python(bundle_dir, python_dir_name, target.minor)
        else:
            _install_host_python_shim(bundle_dir, target.minor)
...
def main() -> None:
    ...
    subparsers.add_parser("bundle", help="Build self-contained bundle")
